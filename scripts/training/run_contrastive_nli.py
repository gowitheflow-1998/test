...
def select_datasets(_dataset, do_select=True):
    key, cls = ('id', 2) if 'id' in _dataset[0] else ('idx', 0)
    if do_select:
        _dataset = _dataset.select(
            data[key] for data in _dataset if data['label'] == cls
        )
    return _dataset
...
def main():
    ...
    this_dataset_name, sentence1_key, sentence2_key = datasets_keys[data_args.dataset_name]
    ...
    if training_args.do_train:
        if data_args.train_file:
            ...
            train_dataset = select_datasets(train_dataset, do_select=False)
        try:
    ...
    if training_args.do_eval:
        if data_args.validation_file:
            ...
            eval_dataset = load_dataset(
                this_dataset_name,
                data_args.dataset_config_name,
                split="validation" if data_args.dataset_name == 'mnli' else 'test',
                cache_dir=model_args.cache_dir,
                use_auth_token=True if model_args.use_auth_token else None,
            )
        try:
    ...
    trainer = PIXELTrainerForContrastive(
        model=model,
        args=training_args,
